...
class DivergenceLoss(nn.Module):
    """Same virtual-hyper-joint divergence term used by official main.py."""
    ...
    def forward(self, x):
        v, _ = x[0].size()
        if v <= 1:
            # hyper_joints=1 has no pairwise diversity term.
            return x[0].new_zeros(())
        loss = x[0].new_zeros(())
        for item in x:
            norm = torch.norm(item, dim=-1, keepdim=True, p=2)
            norm = norm @ norm.T
            similarity = (item @ item.T) / (norm + 1e-8)
            positive = self.relu(similarity)
            loss_i = (positive.sum() - v) / (v * (v - 1))
            loss = loss + loss_i
        return loss / len(x)
...
def train_fold(x, y, groups, args, device, model_cls, seed):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb, yb in fit_loader:
            ...
            loss = ce_loss + args.divergence_weight * h_loss
            loss.backward()
            if args.grad_clip > 0:
            ...
            ce_loss_sum += float(ce_loss.detach()) * n
            h_loss_sum += float(h_loss.detach()) * n
        ...
        train_ce = ce_loss_sum / max(seen, 1)
        train_h = h_loss_sum / max(seen, 1)
...
def dataset_run_name(data_root: Path) -> str:
    ...
    return "hypergcn_normalized" if "normalized" in name else "hypergcn"
...
def main():
    ...
    if args.hyper_joints < 2:
        raise ValueError(
            "Use --hyper-joints >= 2 because official DivergenceLoss is pairwise"
        )
    ...
    if args.result_root == Path("results/hypergcn"):
    ...
    config.update(
        {
            ...
            "classification_loss": "weighted CrossEntropyLoss + label_smoothing",
            "hypergraph_regularizer": "official DivergenceLoss",
        }
    )
    ...
    metrics_lines = [
        "=" * 80,
        "MODEL: Hyper-GCN (official ICCV 2025 base implementation)",
        "=" * 80,
        ...
        f"divergence_weight: {args.divergence_weight}",
        "optimizer: SGD(momentum=0.9,nesterov=True)",
        f"folds: {args.folds}",
        f"seed: {args.seed}",
        f"device: {device}",
    ...
    for fold, (train_idx, test_idx) in enumerate(
        ...
        torch.save(
            {
                "model_type": "hypergcn_official_base_urfall",
                "official_model": "model.hypergcn_base.Model",
                "num_class": NUM_CLASS,
                "num_point": NUM_POINT,
                "num_person": NUM_PERSON,
